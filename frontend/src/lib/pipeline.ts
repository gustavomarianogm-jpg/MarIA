...
const anthropic = new Anthropic({
  apiKey: import.meta.env.VITE_ANTHROPIC_API_KEY || 'missing_key_para_nao_crashar',
...
export async function chamarEntrevistadora(historico: ChatMessage[]) {
  const response = await anthropic.messages.create({
    model: 'claude-sonnet-4.6', // modelo de conversa (pode ser ajustado)
    max_tokens: 1500,
    temperature: 0.7,
  ...
  const prontoParaRelease = texto.includes('[SINAL_INTERNO: PRONTO_PARA_RELEASE]');
  const respostaLimpa = texto.replace('[SINAL_INTERNO: PRONTO_PARA_RELEASE]', '').trim();
