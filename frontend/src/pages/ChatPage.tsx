...
export function ChatPage({
    ...
    return [
      {
        ...
        role: 'assistant',
        content:
          'Oi! 👋 Sou a MarIA, jornalista. Tô aqui pra transformar a sua história em uma matéria que jornalista quer publicar. Antes de a gente começar: me conta seu nome e o que você faz?',
      },
    ];
  });
  ...
  const handleSend = async (
    ...
    audioData?: { url: string; duration: string }
  ) => {
  ...
  const handleGenerateRelease = async (currentMsgs = messages) => {
    ...
    try {
      ...
      const res = await generateReleaseMutation.mutateAsync({
      ...
      if (!isCongressMode) {
        // 5s dá tempo de ler o release antes do paywall interromper.
        setTimeout(() => setIsPaywallOpen(true), 5000);
      }
    } catch (err: unknown) {
      const e = err as { data?: { message?: string }; message?: string };
  ...
  const quickRepliesByStep: Record<number, string[]> = {
    ...
    3: [
      'Sobrevivi a uma crise',
      'Sou pioneiro no setor',
      'Resolvo uma dor real',
    ],
  ...
  return (
    <div
      className="grain w-full overflow-hidden"
      style={{
        flex: 1,
        display: 'flex',
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          width: '100%',
          margin: '0 auto',
          boxShadow: '0 0 60px rgba(15,10,26,0.08)',
        }}
      >
        {/* HEADER */}
        <header
          className="border-b-2 border-ink bg-paper px-4 py-3 flex items-center justify-between flex-shrink-0"
          style={{
            borderBottom: '2px solid #0F0A1A',
        ...
        >
          ...
          >
            <button
              onClick={() =>
                onNavigate(session && !isCongressMode ? 'dash' : 'landing')
              }
            ...
            <div
              className="w-11 h-11 rounded-full bg-ink text-paper flex items-center justify-center font-display font-bold text-lg"
              style={{
                width: '2.75rem',
                height: '2.75rem',
                borderRadius: '50%',
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <div
                className="text-xs text-ink/60 font-mono"
                style={{
                  fontSize: '0.75rem',
          ...
          >
            {isCongressMode && (
              <span className="chat-stamp hidden sm:inline-block">Demo</span>
            )}
            {session && (
              <span className="chat-stamp hidden sm:inline-block">
                {credits} créditos
              </span>
            )}
          </div>
        ...
        {/* PROGRESS BAR */}
        <div
          className="px-4 py-2.5 bg-white flex-shrink-0"
          style={{
            borderBottom: '2px solid #0F0A1A',
        ...
        >
          ...
          >
            ...
            >
              {releaseReady
                ? '✓ RELEASE GERADO'
                : `${Math.min(currentStep + 1, 7)} / 7 · ${steps[Math.min(currentStep, 6)]?.label}`}
            </span>
          </div>
          <div
          ...
          >
            {Array.from({ length: 7 }).map((_, i) => (
              <div
                key={i}
                className={`chat-step-dot ${i < currentStep ? 'done' : i === currentStep ? 'active' : ''}`}
              ></div>
            ))}
          </div>
        </div>
        ...
        {/* MESSAGES AREA */}
        <main
          className="chat-scroll bg-paper relative"
          role="log"
          aria-label="Conversa com a MarIA"
          aria-live="polite"
          style={{
            flex: 1,
            overflowY: 'auto',
        ...
        >
          {messages.map((m) => (
            <div
              key={m.id}
              className={
                m.role === 'user' ? 'chat-bubble-user' : 'chat-bubble-maria'
              }
              style={m.isFinalRelease ? { maxWidth: '92%' } : {}}
            >
              {m.isFinalRelease ? (
                  ...
                  <div className="bg-white rounded-xl shadow-md p-6 sm:p-8 mt-2 relative overflow-hidden border border-ink/10">
                    ...
                    <div className="flex items-center gap-3 mb-8">
                      ...
                      <div>
                        ...
                        <div className="font-mono text-[10px] text-ink/50 mt-1 uppercase tracking-widest">
                          Pronto para envio
                        </div>
                      </div>
                    </div>
                ...
                <div className="chat-audio-msg">
                  ...
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M19 11a7 7 0 01-14 0m7 7v4m0-4a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z"
                    />
                  </svg>
                ...
                <div>
                    ...
                    <div className="flex flex-col gap-2 mb-2">
                      {m.files.map((f, i) =>
                        f.type.startsWith('image/') ? (
                          ...
                          >
                            <span className="text-xl">📄</span> {f.name}
                          </div>
                        )
                      )}
                    </div>
                  ...
                  <ReleaseMarkdown
                    content={m.content.replace(
                      /\[(Arquivos anexados|Áudio gravado).*?\]\n/,
                      ''
                    )}
                    variant="chat"
                    role={m.role}
          ...
          {(chatMutation.isPending || generateReleaseMutation.isPending) && (
            <div
              className="chat-bubble-maria chat-typing"
              role="status"
              aria-live="polite"
        ...
        {!isRecording &&
          ...
          !input &&
          quickRepliesByStep[currentStep] &&
          quickRepliesByStep[currentStep].length > 0 && (
            <div
        ...
        {/* INPUT BAR */}
        <footer
          className="border-t-2 border-ink bg-paper px-3 py-3 flex-shrink-0"
          style={{
            borderTop: '2px solid #0F0A1A',
            padding: '0.75rem',
            flexShrink: 0,
          }}
        >
          {!isRecording ? (
            ...
            >
              ...
              <div
                ...
                style={{
                  ...
                  border: '1.5px solid #0F0A1A',
                  borderRadius: '1rem',
                  boxShadow: '3px 3px 0 rgba(15,10,26,0.12)',
                  display: 'flex',
                  alignItems: 'flex-end',
                }}
              >
            ...
            >
              ...
              >
                <span
                  className="chat-recording chat-input-btn"
                  style={{
                    width: '14px',
                    height: '14px',
                    border: 'none',
                ...
                <span
                  className="text-xs text-ink/50 font-mono"
                  style={{
                    fontSize: '0.75rem',
                    color: 'rgba(15,10,26,0.5)',
                    fontFamily: "'JetBrains Mono', monospace",
                  }}
                >
                  gravando áudio...
          ...
          <div
            className="mt-2 text-center text-[10px] font-mono text-ink/40 tracking-wider"
            style={{
              marginTop: '0.75rem',
              textAlign: 'center',
              fontSize: '10px',
          ...
          >
            MarIA é uma assessora virtual · Suas respostas geram um release
            profissional
          </div>
        </footer>
      </div>
      ...
      {/* PAYWALL MODAL */}
      <div
        className={`modal-overlay ${isPaywallOpen ? 'open' : ''}`}
        role="dialog"
        aria-modal="true"
        aria-labelledby="paywall-title"
        aria-hidden={!isPaywallOpen}
        onClick={(e) => {
          if (e.target === e.currentTarget) setIsPaywallOpen(false);
        }}
      >
        <div className="modal-card grain">
          ...
          <h2
            id="paywall-title"
            className="display text-3xl mb-2 relative z-10"
            style={{
              fontWeight: 600,
              fontSize: '1.875rem',
              lineHeight: '2.25rem',
          ...
          >
            Seu release está pronto.{' '}
            <span style={{ color: '#E91E8C' }}>Falta o último passo.</span>
          </h2>
          <p
            className="text-sm text-ink/70 leading-relaxed mb-5 relative z-10"
            style={{
              fontSize: '0.875rem',
              color: 'rgba(15,10,26,0.7)',
              lineHeight: 1.625,
              marginBottom: '1.25rem',
            }}
          >
            Sua história já está no formato que jornalista lê. Para a nossa
            curadoria revisar e{' '}
            <strong>enviar para os jornalistas certos</strong>, escolha como
            você quer seguir.
          </p>

          <div
            className="flex flex-col gap-3 mb-5 relative z-10"
            style={{
              display: 'flex',
              flexDirection: 'column',
              gap: '0.75rem',
          ...
          >
            <div className="plan-card selected" style={{ cursor: 'default' }}>
              ...
              >
                ...
                <span
                  className="font-mono text-xs bg-ink text-paper px-2 py-0.5 rounded-md"
                  style={{
                    fontFamily: "'JetBrains Mono', monospace",
                    fontSize: '0.75rem',
                    backgroundColor: '#0F0A1A',
              ...
              <div
                className="display text-4xl my-1"
                style={{
                  fontWeight: 600,
                  fontSize: '2.25rem',
                  lineHeight: '2.5rem',
              ...
              >
                R$ 297
                <span
                  className="text-base font-sans text-ink/60"
                  style={{
                    fontSize: '1rem',
                    fontFamily: "'Inter Tight', sans-serif",
                    color: 'rgba(15,10,26,0.6)',
              ...
              >
                <span
                  className="font-mono text-xs bg-rose text-paper px-2 py-0.5 rounded-md font-semibold"
                  style={{
                    fontFamily: "'JetBrains Mono', monospace",
                    fontSize: '0.75rem',
              ...
              <div
                className="news-rule my-3 relative z-10"
                style={{ marginTop: '0.75rem', marginBottom: '0.75rem' }}
              ></div>
              <ul
                className="flex flex-col gap-2 text-xs text-ink/80"
                style={{
                  display: 'flex',
                  flexDirection: 'column',
                  gap: '0.5rem',
                  fontSize: '0.75rem',
                  color: 'rgba(15,10,26,0.8)',
                }}
              >
                <li
                ...
                >
                  <svg
                    className="w-4 h-4 flex-shrink-0 mt-0.5 text-rose"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2.5"
                    viewBox="0 0 24 24"
          ...
          <div
            className="flex items-center gap-2 mb-4 text-xs text-ink/70 font-mono relative z-10"
            style={{
              display: 'flex',
              alignItems: 'center',
              gap: '0.5rem',
          ...
          >
            <svg
              className="w-4 h-4 flex-shrink-0 text-rose"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
              viewBox="0 0 24 24"
          ...
          <button
            className="modal-cta relative z-10"
            onClick={() => {
              alert('Redirecionando para o Stripe/Asaas: Plano Mensal');
          ...
          <button
            onClick={() => setIsPaywallOpen(false)}
            className="w-full text-center text-xs text-ink/50 font-mono mt-3 hover:text-ink transition-colors relative z-10 bg-transparent border-none cursor-pointer"
            style={{
              width: '100%',
              textAlign: 'center',
              fontSize: '0.75rem',
