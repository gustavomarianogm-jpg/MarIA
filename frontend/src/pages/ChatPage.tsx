...
export function ChatPage({ session, onNavigate }: { session: any, onNavigate: (r: string) => void }) {
  ...
  const handleGenerateRelease = async () => {
    ...
    try {
      ...
      const { resposta, prontoParaRelease } = await processarTurnoChat(messages);
  ...
  if (release) {
    return (
      <div className={styles.pageFull} style={{ paddingTop: isCongressMode ? '20px' : '60px' }}>
        <div className={styles.chatOuter} style={{ maxWidth: '800px', margin: '20px auto' }}>
  ...
  return (
    <div className={styles.pageFull} style={{ paddingTop: isCongressMode ? '0px' : '60px' }}>
      <div className={styles.chatOuter}>
        ...
        <div className={styles.msgsContainer}>
          {messages.map((m, i) => (
            <div key={i} className={`${styles.messageBubble} ${m.role === 'user' ? styles.user : styles.assistant}`}>
        ...
        {canGenerateRelease ? (
          ...
          >
            {isProcessing ? '⏳ Escrevendo Release...' : `✨ Gerar release profissional ${session ? '(1 crédito)' : '(Grátis)'}`}
          </button>
        ) : (
          <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', padding: '10px 20px 24px', width: '100%', maxWidth: '840px', margin: '0 auto', justifyContent: 'center', flexShrink: 0 }}>
        ...
        <form onSubmit={handleSend} className={styles.inputForm}>
          ...
          />
          <button type="submit" aria-label="Enviar mensagem" disabled={!input.trim() || chatMutation.isPending || isProcessing} className={styles.sendButton}>
